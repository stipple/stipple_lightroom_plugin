...
-- publishServiceProvider.disableRenamePublishedCollection = true -- not used for Stipple sample plug-in
...
-- publishServiceProvider.disableRenamePublishedCollectionSet = true -- not used for Stipple sample plug-in
...
function publishServiceProvider.renamePublishedCollection( publishSettings, info )
  if info.remoteId then
    StippleAPI.createOrUpdatePhotoset(publishSettings, {
...
function publishServiceProvider.deletePublishedCollection( publishSettings, info )
  import 'LrFunctionContext'.callWithContext( 'publishServiceProvider.deletePublishedCollection', function( context )
    ...
    local progressScope = LrDialogs.showModalProgressDialog {
      title = LOC( "$$$/Stipple/DeletingCollectionAndContents=Deleting photoset ^[^1^]", info.name ),
    ...
    if info and info.photoIds then
      for i, photoId in ipairs( info.photoIds ) do           
        ...
        StippleAPI.deletePhoto( publishSettings, { photoId = photoId } )
      end        		
    end
      	
    if info and info.remoteId then    	
      StippleAPI.deletePhotoset(publishSettings, {
        photosetId = info.remoteId,
        suppressError = true,
      })    	
    end            			
...
function publishServiceProvider.getCommentsFromPublishedCollection( publishSettings, arrayOfPhotoInfo, commentCallback )
  for i, photoInfo in ipairs( arrayOfPhotoInfo ) do
    local comments = StippleAPI.getComments(publishSettings, {
...
publishServiceProvider.titleForPhotoRating = LOC "$$$/Stipple/TitleForPhotoRating=Favorite Count"
...
function publishServiceProvider.canAddCommentsToService( publishSettings )
  return StippleAPI.testStippleConnection( publishSettings )
end
